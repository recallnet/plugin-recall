...
export async function logMemoryPostgres(
  ...
  const roomCheck = await db.query("SELECT id FROM rooms WHERE id = $1", [
    params.roomId,
  ]);
...
export async function markLogsAsSyncedPostgres(
  db: any,
  logIds: UUID[],
): Promise<void> {
  if (logIds.length === 0) {
...
export async function getUnsyncedLogsSqlite(
  db: any,
): Promise<{ id: UUID; body: string }[]> {
  const sql =
    "SELECT id, type, body FROM logs WHERE isSynced = 0 ORDER BY createdAt ASC";
  return db.prepare(sql).all() as { id: UUID; body: string }[];
}

export async function markLogsAsSyncedSqlite(
  db: any,
  logIds: UUID[],
): Promise<void> {
